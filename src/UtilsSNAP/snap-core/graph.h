...
public:
  ...
  /// Deletes an edge from node IDs SrcNId to DstNId from the graph. ##TNGraph::DelEdge
  bool DelDirEdge(const int& SrcNId, const int& DstNId);
  /// Tests whether an edge from node IDs SrcNId to DstNId exists in the graph.
  bool IsEdge(const int& SrcNId, const int& DstNId, const bool& IsDir = true) const;
  /// MINE !!!!!
  bool IsEdgeUnchecked(const int& SrcNId, const int& DstNId) const;
  bool IsEdgeDirUnchecked(const int& SrcNId, const int& DstNId) const;
  bool IsEdgeRevUnchecked(const int& SrcNId, const int& DstNId) const;
  /// Tests whether an edge EId exists in the graph (for compatibility with TNEANet), always returns false.
  bool IsEdge(const int& EId) const { return false; }
  /// Returns an iterator referring to the first edge in the graph.
