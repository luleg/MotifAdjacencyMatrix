...
/// Loads a (directed, undirected or multi) graph from a text file InFNm with 1 edge per line (whitespace separated columns, arbitrary string node ids). 1st int provided
template <class PGraph> PGraph LoadEdgeListStr(const TStr& InFNm, const int& SrcColId, const int& DstColId, TStrHash<TInt>& StrToNIdH, const int& first_ind) ;
template <class PGraph> PGraph LoadEdgeListStr(const TStr& InFNm, const int& SrcColId, const int& DstColId, TStrHash<TInt>& StrToNIdH, const int& first_ind, const char& Separator);
...
/// Loads the format saved by TSnap::SaveEdgeList(), where node IDs are strings and mapping of strings to node ids are stored. First Id of node is provided
/// ny first_ind ##LoadEdgeListStr_StrToNIdH
template <class PGraph>
PGraph LoadEdgeListStr(const TStr& InFNm, const int& SrcColId, const int& DstColId, TStrHash<TInt>& StrToNIdH, const int& first_ind) {
  TInt cpt(0);
  for (TInt tmp=0;tmp<first_ind;tmp++){
    TStr dmy = tmp.GetStr();
    StrToNIdH.AddKey(dmy);
  }
  TSsParser Ss(InFNm, ssfWhiteSep);
  PGraph Graph = PGraph::TObj::New();
  while (Ss.Next()) {
...
/// ny first_ind ##LoadEdgeListStr_StrToNIdH
template <class PGraph>
PGraph LoadEdgeListStr(const TStr& InFNm, const int& SrcColId, const int& DstColId, TStrHash<TInt>& StrToNIdH, const int& first_ind, const char& Separator) {
  TInt cpt(0);
  for (TInt tmp=0;tmp<first_ind;tmp++){
    TStr dmy = tmp.GetStr();
    StrToNIdH.AddKey(dmy);
