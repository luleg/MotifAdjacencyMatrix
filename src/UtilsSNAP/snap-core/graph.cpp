...
bool TNGraph::DelDirEdge(const int& SrcNId, const int& DstNId) {
  bool EdgeExist=false;
  TNode& SrcNode = GetNode(SrcNId);
  const int fromSrc = SrcNode.OutNIdV.SearchBin(DstNId);
  if (fromSrc!= -1) {
    EdgeExist = true;
    SrcNode.OutNIdV.Del(fromSrc);
    // TNode& DestNode = GetNode(DstNId);
    // const int fromDest = DestNode.InNIdV.SearchBin(SrcNId);
    // DestNode.InNIdV.Del(fromDest);
  }
  return EdgeExist;
}
...
bool TNGraph::IsEdgeDirUnchecked(const int& SrcNId, const int& DstNId) const {
  return GetNode(SrcNId).IsOutNId(DstNId);
}

bool TNGraph::IsEdgeRevUnchecked(const int& SrcNId, const int& DstNId) const {
  return GetNode(SrcNId).IsInNId(DstNId);
}
bool TNGraph::IsEdgeUnchecked(const int& SrcNId, const int& DstNId) const {
